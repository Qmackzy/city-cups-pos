    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <button onclick="document.getElementById('modalTambah').classList.remove('hidden')"
                class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition font-bold text-sm shadow-sm">
                + Tambah Bahan Baru
            </button>
        </div>
    </x-slot>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm">
                            @forelse($ingredients as $ing)
                                <tr
                                    class="hover:bg-gray-50 transition border-b last:border-0 {{ $ing->stock <= $ing->min_stock ? 'bg-red-50' : '' }}">
                                    <td class="p-4">
                                        <div class="font-bold text-gray-800">{{ $ing->name }}</div>
                                        <div class="text-xs text-gray-400 font-medium lowercase">Satuan:
                                            {{ $ing->unit }}</div>
                                    </td>
                                    <td
                                        class="p-4 font-black text-lg {{ $ing->stock <= $ing->min_stock ? 'text-red-600' : 'text-gray-700' }}">
                                        {{ number_format($ing->stock) }} <span
                                            class="text-xs font-normal text-gray-400">{{ $ing->unit }}</span>
                                    </td>
                                    ...
                                    <td class="p-4">
                                        @if ($ing->stock <= $ing->min_stock)
                                            <span
                                                class="px-2 py-1 bg-red-100 text-red-600 rounded-full text-[10px] font-black uppercase tracking-wider animate-pulse">Kritis</span>
                                        @else
                                            <span
                                                class="px-2 py-1 bg-green-100 text-green-600 rounded-full text-[10px] font-black uppercase tracking-wider">Aman</span>
                                        @endif
                                    </td>
                                    <td class="p-4">
                                        <div class="flex justify-center gap-2">
                                            <form action="{{ route('ingredients.addStock', $ing->id) }}" method="POST"
                                                class="flex gap-1">
                                                @csrf
                                                <input type="number" name="amount" placeholder="Qty"
                                                    class="w-16 text-xs border-gray-300 rounded focus:ring-blue-500 py-1"
                                                    required>
                                                <button type="submit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="p-10 text-center text-gray-400 italic">Belum ada bahan
                                        baku. Tambahkan sekarang!</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <form action="{{ route('ingredients.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="w-full border-gray-300 rounded-lg text-sm"
                        placeholder="Contoh: Biji Kopi" required>
                </div>
                <div class="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="unit" class="w-full border-gray-300 rounded-lg text-sm"
                            placeholder="gram / ml" required>
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-red-600">Batas Stok Kritis (Min)</label>
                    <input type="number" name="min_stock" class="w-full border-red-200 bg-red-50 rounded-lg text-sm"
                        required>
                </div>
                ...
                <div class="flex justify-end gap-2">
                    <button type="button" onclick="document.getElementById('modalTambah').classList.add('hidden')"
                        class="px-4 py-2 bg-gray-200 rounded-lg text-sm font-bold">Batal</button>
                    <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold">Simpan</button>
                </div>
            </form>
        </div>
    </div>
