            ...
            <div class="bg-white p-6 rounded-lg shadow mb-6">
                ...
                <form action="{{ route('waste.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="ingredient_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                            required>
                    ...
                    <div>
                        ...
                        <input type="text" name="reason" placeholder="Contoh: Tumpah / Basi"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full bg-red-600 text-white py-2 px-4 rounded-md hover:bg-red-700">Simpan
                            Kerugian</button>
                    </div>
                </form>
            </div>
                    ...
                    <thead>
                        <tr>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                                Tanggal</th>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Bahan
                                Baku</th>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                                Jumlah</th>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($wastes as $w)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ $w->amount }} {{ $w->ingredient->unit }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-red-600">Rp
                                    {{ number_format($w->loss_amount) }}</td>
