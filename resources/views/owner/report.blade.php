    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <form action="{{ route('owner.laporan') }}" method="GET" class="flex flex-wrap items-end gap-4">
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ $startDate }}" class="border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Tanggal Sampai</label>
                            <input type="date" name="end_date" value="{{ $endDate }}" class="border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 text-sm">
                        </div>
                        <button type="submit" class="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition font-bold text-sm shadow-sm">
                            Filter Laporan
                        </button>
                    </form>
                    ...
                    <form action="{{ route('reports.download') }}" method="GET" target="_blank">
                        ...
                        <button type="submit" class="flex items-center gap-2 bg-red-600 text-white px-5 py-2 rounded-lg hover:bg-red-700 transition shadow-sm font-bold text-sm">
                    ...
                    <a href="{{ route('owner.laporan') }}" class="bg-gray-100 text-gray-600 px-5 py-2 rounded-lg hover:bg-gray-200 transition font-bold text-sm border border-gray-200">
                        Reset
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="{{ $netProfit >= 0 ? 'bg-green-600' : 'bg-red-700' }} p-5 rounded-xl text-white shadow-lg border-2 border-white border-opacity-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-white p-6 rounded-xl shadow-sm border-l-4 border-blue-500 border border-gray-100">
                    <h3 class="text-gray-500 text-xs font-bold uppercase tracking-wider">Pendapatan (Filter Terpilih)</h3>
                    <p class="text-3xl font-black text-gray-800 mt-1">Rp {{ number_format($totalRevenue) }}</p>
                    <p class="text-xs text-gray-400 mt-2 italic font-medium">Periode: {{ date('d M Y', strtotime($startDate)) }} - {{ date('d M Y', strtotime($endDate)) }}</p>
                </div>
                        ...
                        <tbody class="text-sm">
                            ...
                            <tr class="hover:bg-gray-50 transition border-b last:border-0 text-gray-700">
                                ...
                                <td class="p-4 text-center">
                                    <a href="{{ route('kasir.print', $trans->id) }}" 
                                       target="_blank" 
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="p-10 text-center text-gray-400 italic">Tidak ada transaksi di periode ini.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
